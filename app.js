...
async function handleAttendance(action) {
    ...
    try {
        ...
        try {
            ...
            address = distanceInfo + geoData.display_name;
        } catch (e) { 
            console.error('Address lookup failed', e);
            address = distanceInfo + 'Không xác định được địa chỉ';
        }
...
function showMap(lat, lng, accuracy, addr, name) {
    ...
    const distMatch = addr.match(/\(Cách công ty (.*?)\)/);
    const distText = distMatch ? distMatch[1] : '---';
    ...
    info.innerHTML = `
        ...
        <div style="font-weight: 600; color: var(--text-main); margin-bottom: 8px;">
            ...
            <span style="color: var(--success); font-size: 1.1rem;">${distText}</span>
        </div>
        <div style="font-weight: 600; color: var(--text-main); margin-bottom: 5px;">Địa chỉ thực tế:</div>
        <div style="margin-bottom: 8px;">${addr.replace(/\(Cách công ty .*?\)/, '') || 'Không xác định được địa chỉ'}</div>
